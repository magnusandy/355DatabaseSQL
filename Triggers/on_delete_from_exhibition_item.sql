...
begin
	...
	IF(next_location IS NOT NULL ) THEN
		...
		RAISE NOTICE 'Next location is not null';
		IF(previous_location = 'Storage') THEN
			...
			UPDATE t_item_locations SET ilo_ilodatetime_end = 
			(	
				SELECT ilo_datetime_start FROM t_item_locations
				WHERE
				(
					ilo_inumkey = OLD.exi_inumkey
					AND
	...
	ELSE
		...
		RAISE NOTICE 'Is in a location after this';
		IF(previous_location = 'Storage') THEN
